...
# ========================输入区开始========================
search_writer_name = 'Brian Buccellato'  # 查询用漫画名

url_prefix = 'https://www.comixology.com/search?search='
comic_url = url_prefix + search_writer_name  # 完整的查询网址
# ========================执行区开始========================
    ...
    if re.match(r'.*/digital-comic/[^?]*', all_url[i]):  # 合法漫画页链接
        ...
        if short_link not in check_set:
            ...
            if writer == search_writer_name:
                ...
                text = "\r\n".join(text_list)
                # ================写入TXT================
                file_name = 'Comixology搜索+简介-编剧' + search_writer_name + '.txt'
