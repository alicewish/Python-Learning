...
import requests, urllib.parse, time, xlwt

start_time = time.time()  # 初始时间戳
# ========================输入区开始========================
user = 'Nemesis43'  # 用户名

file_name = user + '地址'  # 文件名
search_domain = 'http://kickasstorrentsan.com/user/'  # 搜索的网站地址
path_prefix = '/Users/alicewish/我的坚果云/新'  # 文件地址前缀
...
# ==================获取最后一页项数==================
thispage = requests.get(search_domain + user + '/uploads/?page=' + str(page_number))
tree = html.fromstring(thispage.text)
...
for i in range(page_number):
    ...
    thispage = requests.get(search_domain + user + '/uploads/?page=' + str(i + 1))
    tree = html.fromstring(thispage.text)
    dataparams = tree.xpath('//div[@class="none"]/@data-sc-params')
